    ...
    $select_query = mysqli_query($conn, "SELECT * FROM tb_menu WHERE nama = '$nama_menu'");
        if (mysqli_num_rows($select_query) > 0) {
                ...
                if(move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                    $query = mysqli_query($conn, "INSERT INTO tb_menu (nama, foto, keterangan , kategori , nama_toko , harga , stok ) 
                    VALUES ('$nama_menu', '".$kode_rand.$_FILES['foto']['name']."', '$keterangan', '$kategori_menu', '$kios', '$harga', '$stok')");
                    if ($query) {
